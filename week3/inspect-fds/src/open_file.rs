...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let name = OpenFile::path_to_name(fs::read_link(link_path).ok()?.to_str()?);
        let content = fs::read_to_string(fd_path).ok()?;
        Some(OpenFile::new(name, Self::parse_cursor(&content).unwrap(), Self::parse_access_mode(&content).unwrap()))
    }
