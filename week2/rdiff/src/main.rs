...
fn main() {
    ...
    let mut v1 = read_file_lines(filename1).expect("can not open file 1.");
    let mut v2 = read_file_lines(filename2).expect("can not open file 2.");

    let c = lcs(&v1, &v2);
    print_diff(&c, &v1, &v2, v1.len(), v2.len());
}
